...
# Education vs. Crime
ggplot(data = data_clean, mapping = aes(x = PctLess9thGrade, y = ViolentCrimesPerPop)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "lm", color = "purple") +
  labs(title = "Violent Crime vs. % With Less Than A 9th Grade Education",
       x = "% With Less Than A 9th Grade Education",
...
ggplot(data = data_clean, mapping = aes(x = PctBSorMore, y = ViolentCrimesPerPop)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "lm", color = "skyblue") +
  labs(title = "Violent Crime vs. % With A Bachelors Degree Or Higher Education",
       x = "% With A Bachelors Degree Or Higher Education",
       y = "Violent Crime Rate")
...
# ~ cor(., data_clean$ViolentCrimesPerPop, use = "complete.obs")
# . represents the current column.
#	cor(x, y) computes Pearson correlation.
#	use = "complete.obs" removes rows containing NA in either variable.
...
# Creates the beginning of the regression formula.
# "~" means predict using in R.
# paste(top15_vars, collapse = " + ") turns the vector into a single string:
# paste("ViolentCrimesPerPop ~", ...) combines the left-hand side (dependent variable) and the predictors
...
# independent variables: the 15 predictors
# dataset: communities
full_adj_r2 <- summary(model_full)$adj.r.squared # $adj.r.squared pulls the adjusted R² value
full_adj_r2
...
#reformulate(rhs_vector, lhs_string) is a convenience function that creates a formula from character vectors.
